...
async function startDev() {
  ...
  const useTunnel = process.env.USE_TUNNEL !== 'false'; // Default to true
  let miniAppUrl;
  ...
  if (useTunnel) {
    try {
      ...
      tunnel = { url: tunnelUrl, close: () => ngrok.kill() };
      
      miniAppUrl = tunnelUrl;
      console.log(`
🌐 Ngrok tunnel URL: ${tunnelUrl}

💻 To test on desktop:
   1. Open the tunnel URL in your browser: ${tunnelUrl}
   2. Navigate to the Warpcast Mini App Developer Tools: https://warpcast.com/~/developers
   3. Enter your mini app URL: ${tunnelUrl}
